// Migration script to move mock data to Supabase
// Run this script once to populate your Supabase database with initial data

import { supabase } from '../lib/supabase';

// Mock data from your original opportunitiesApi
const mockOpportunities: Array<{
  ...
  createdAt: string;
  status: string;
  companyId?: string; // Add this property for migration
}> = [
  {
    id: '1',
...
async function migrateCompanies() {
  ...
  for (const company of mockCompanies) {
    try {
      ...
      // Update the mock opportunity to use the real company ID
      const opportunityIndex = mockOpportunities.findIndex(
        opp => opp.companyName === company.name
      );
      if (opportunityIndex !== -1) {
        mockOpportunities[opportunityIndex].companyId = newCompany.id;
      }
    } catch (error) {
...
// Function to migrate opportunities
async function migrateOpportunities() {
  ...
  for (const opportunity of mockOpportunities) {
    try {
      // Skip if no company ID (migration failed)
      if (!opportunity.companyId) {
        console.log(`Skipping opportunity ${opportunity.title} - no company ID`);
        continue;
      }
      ...
      const { data: existingOpportunity } = await supabase
        ...
        .eq('created_by_id', opportunity.companyId)
        .single();
      ...
      const opportunityData = {
        ...
        type: opportunity.type.toUpperCase().replace('-', '_') as any,
        experience_level: opportunity.experienceLevel.toUpperCase() as any,
        status: opportunity.status.toUpperCase() as any,
        created_by_id: opportunity.companyId,
      };
...
if (typeof window !== 'undefined') {
  // In browser environment, you can call runMigration() manually
  console.log('Migration script loaded. Call runMigration() to start migration.');
} else {
  // In Node.js environment, run automatically
  runMigration();
}
